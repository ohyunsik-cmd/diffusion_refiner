...
def new_gram_matrix(tensor):
    # tensor: (B, C, H, W)
    B, C, H, W = tensor.shape
    x = tensor.view(B, C, H * W)
    G = x @ x.transpose(1, 2)          # (B, C, C)
    return G / (C * H * W)
...
def gram_loss(style, target, model):
    ...
    style_grams = {layer: new_gram_matrix(style_features[layer]) for layer in style_features}
    ...
    for layer, weight in STYLE_WEIGHTS.items():
        ...
        target_gram = new_gram_matrix(target_feature)
